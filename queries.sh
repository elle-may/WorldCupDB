...
echo "$($PSQL "SELECT name FROM (SELECT winner_id AS team_id FROM games WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT opponent_id AS team_id FROM games WHERE year = 2014 AND round = 'Eighth-Final') q1 INNER JOIN teams on q1.team_id = teams.team_id ORDER BY name")"
